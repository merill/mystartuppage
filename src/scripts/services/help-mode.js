...
globalThis.window.addEventListener('load', function () {
    // if page hasn't loaded after 5s, shows prompt
...
function exportToJsonFile(json) {
    ...
    a.href = url
    a.download = `mystartuppage-${new Date().toLocaleString()}.json`
    a.click()
...
async function getDataAsString() {
    ...
    return localStorage.mystartuppage ?? ''
}
...
async function resetApply() {
    ...
    const archiveName = `mystartuppage-archive-${new Date().toLocaleString()}`
    ...
    if (localStorage) {
        Object.keys(localStorage).forEach((key) => {
            if (key.startsWith('mystartuppage-archive-') === false) {
...
function createHelpModeDisplay() {
    ...
    // Server statuses (no external services used)
