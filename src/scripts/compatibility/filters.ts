    ...
    for (const link of links) {
        ...
        if (link.icon && typeof link.icon === 'string') {
            ...
            const faviconWasAutomatic = icon.startsWith('https://services.bonjourr.fr') ||
                icon.startsWith('https://t3.gstatic.com') || isNumber(icon)
            ...
            const autoValue = faviconWasAutomatic && isNumber(icon) ? undefined : undefined
...
export function toggleMoveWidgets(current: Sync, imported: Import): Sync {
    ...
    if (!imported.move) {
        ...
        if (Object.keys(imported).some((key) => key.match(/time|main|notes|quicklinks/g))) {
