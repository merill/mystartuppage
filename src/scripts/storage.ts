...
async function syncGet(key?: string | string[]): Promise<Sync> {
    switch (storage.type.get()) {
        ...
        default: {
            return verifyDataAsSync(parse<Sync>(localStorage.mystartuppage) ?? {})
        }
    }
}
...
async function syncSet(keyval: Record<string, unknown>, fn = () => {}): Promise<void> {
    ...
    switch (storage.type.get()) {
        ...
        case 'localstorage': {
            ...
            const data = verifyDataAsSync(parse<Sync>(localStorage.mystartuppage) ?? {})

            for (const [k, v] of Object.entries(keyval)) {
                data[k] = v
            }

            localStorage.mystartuppage = JSON.stringify(data ?? {})
...
async function syncClear(): Promise<void> {
    switch (storage.type.get()) {
        ...
        case 'localstorage': {
            localStorage.removeItem('mystartuppage')
            return
        }
